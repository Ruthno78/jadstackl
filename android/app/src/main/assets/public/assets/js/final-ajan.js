...
(function () {
  ...
  function buildReceipt(data) {
    data = data || {};
    const width = Number(data.paperWidth || data.printerWidth || 32);
    ...
    const companyName = clean(data.companyName || data.company || (isTest ? 'JADSTACK LOTTO' : CONFIG.name));
    ...
    out += 'Reference ticket: ' + uniqueNumber + '\n';
    out += sequence + '\n';
    out += 'Date: ' + date + '\n';
    out += 'Agent/Boutique : ' + seller + '\n';
        ...
        <article class="vente-panel-v88 vente-saisie-v88">
          ...
          <div class="vente-champ-v88">
            ...
            <div class="vente-jeux-v88" id="vente-jeux" aria-label="Liste des jeux">
              ${(games||[]).map((g,i)=>`<button type="button" class="vente-jeu-v88" data-code="${esc(g.code)}" aria-pressed="${i===0?'true':'false'}">${esc(g.name||g.code)}</button>`).join('')}
            </div>
          </div>
        ...
        back.innerHTML = `
          <div class="modal jl-fiche-preview-modal" role="dialog" aria-modal="true">
            ...
            <div class="jl-fiche-preview-hd">
              ...
              <span class="jl-fiche-cd" id="jl-fiche-cd">3s</span>
            </div>
            <div class="fiche-wrap" style="max-width:320px;margin:.5rem auto 0">${window.Lotri.renderFiche(draft)}</div>
            <button type="button" class="btn btn-primary jl-fiche-continue">Continuer</button>
            <p class="muted jl-fiche-preview-note">La fiche s\'imprime automatiquement dans <span id="jl-fiche-cd2">3</span>s —
              appuyez sur Continuer pour lancer immédiatement ou X pour fermer.</p>
          </div>`;
        document.body.appendChild(back);

        let remaining = 3, cancelled = false, timerId = null;
